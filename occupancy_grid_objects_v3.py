...
class OccupancyGridPublisher(Node):
    ...
    def expand_cells(self, object_cells, expansion_factor, grid_height, grid_width):
        expanded_cells = set()
        for row, col in object_cells:
            for dr in range(-expansion_factor, expansion_factor + 1):
                for dc in range(-expansion_factor, expansion_factor + 1):
                    new_row, new_col = row + dr, col + dc
                    if 0 <= new_row < grid_height and 0 <= new_col < grid_width:
                        expanded_cells.add((new_row, new_col))
        return list(expanded_cells)

    def publish_occupancy_grid(self, object_index, object_cells, grid_height, grid_width, map_info, expansion_factor):
        # Initialize the occupancy grid data
        occupancy_data = [-1] * (grid_height * grid_width)

        # Expand the cells corresponding to the object if expansion_factor > 0
        if expansion_factor > 0:
            expanded_cells = self.expand_cells(object_cells, expansion_factor, grid_height, grid_width)
        else:
            expanded_cells = object_cells

        # Fill in the expanded cells with occupied cells (100)
        ...
        self.get_logger().info(f"Published OccupancyGrid message for object {object_index} with expansion factor {expansion_factor}")
        ...
        while True:
            ...
            try:
                ...
                if 1 <= selected_object_index <= num_objects:
                    # Prompt for expansion factor input each time before publishing
                    while True:
                        expansion_input = input("Enter the expansion factor (0 or positive integer): ")
                        try:
                            expansion_factor = int(expansion_input)
                            if expansion_factor >= 0:
                                break
                            else:
                                self.get_logger().info("Please enter 0 or a positive integer for the expansion factor.")
                        except ValueError:
                    ...
                    self.publish_occupancy_grid(selected_object_index, objects[selected_object_index - 1], height, width, map_info, expansion_factor)
                else:
